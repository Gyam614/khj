...
/* 메서드 정의를 위한 메서드 원형작성법
	...
	2. 메서드를 구현하면 내가 편하다!
		...
		- 이후에는 이전에 만든 메서드를 사용하면 되니 시간이 단축된다.
		- 그래서 메서드도 설계를 따로 해주긴 한다. 
		
	3. 메서드에 필요한 것을 제공 (매개변수)
		-여러 개의 메서드는 서로 협력하여 일을 진행한다.
		-물양동이를 혼자서 200m 걸어가 옮기는 것보다
		-여러 명이 한줄로 서서 전달 > 전달 > 전달... 해서 옮기면 개개인이 덜 힘들다.
		-최종 목적지까지 도달하기 위해 다음 사람에게 필요한 것(양동이-매개변수)를 전달해준다.
		
	4. 다음 할일을 하기 위해 필요한거 돌려받기 (반환 - return)
		- 물을 담아서 양동이를 전달하였다면 계속 일을 하기 윟해
		- 빈 양동이를 다시 받아와야 한다.
